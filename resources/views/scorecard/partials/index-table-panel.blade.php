@php
    ...
    $rankPaginator = $isDivision ? ($divisionRanking ?? null) : ($districtRanking ?? null);
    $rankItems = method_exists($rankPaginator ?? null, 'getCollection')
        ? $rankPaginator->getCollection()->values()
        : collect($rankPaginator ?? [])->values();

    $pageOffset = method_exists($rankPaginator ?? null, 'currentPage')
        ? (($rankPaginator->currentPage() - 1) * $rankPaginator->perPage())
        : 0;
    ...
    $mainRoute = $mainRoute ?? (Route::has('scorecard.index') ? route('scorecard.index') : url()->current());
@endphp

<div class="sc-panel" id="scorecardTablePanel">
    <div class="sc-panel-header">
        <div>
            <h5 class="sc-panel-title" id="tablePanelTitle">{{ $isDivision ? 'Division' : 'District' }} Ranking</h5>
            <div class="sc-panel-subtitle">Click row to highlight on map · Click name to view detail</div>
        </div>
        <div class="d-flex flex-column align-items-end gap-2">
            <div class="text-end sc-muted">
                <strong>{{ method_exists($rankPaginator ?? null, 'total') ? $rankPaginator->total() : $rankItems->count() }}</strong> total
            </div>
    ...
    @if($rankItems->count())
        <div class="table-responsive">
            <table class="sc-table" id="districtTable">
                ...
                <tr>
                    <th class="sc-rank">#</th>
                    <th>{{ $isDivision ? 'Division' : 'District' }}</th>
                    <th>Grade / Score</th>
                    <th>Performance</th>
                </tr>
                </thead>
                <tbody>
                @foreach($rankItems as $row)
                    @php
                        $score = (float) ($row->score_percentage ?? 0);
                        $meta = $scoreMeta($score);
                        $areaName = $isDivision ? (optional($row->division ?? null)->name ?? 'N/A') : (optional($row->district ?? null)->name ?? 'N/A');
                    ...
                    <tr data-area="{{ strtoupper($areaName) }}"
                        ...
                        <td>
                            ...
                            <div class="sc-muted">Punjab {{ $isDivision ? 'Division' : 'District' }}</div>
                        </td>
                        <td>
        ...
        @if(method_exists($rankPaginator ?? null, 'hasPages') && $rankPaginator->hasPages())
            <div class="sc-pagination-wrap">
                <div class="sc-muted">Showing {{ $rankPaginator->firstItem() }}–{{ $rankPaginator->lastItem() }} of {{ $rankPaginator->total() }}</div>
                {{ $rankPaginator->appends(request()->query())->links('pagination::bootstrap-5') }}
            </div>
        @endif
    @else
