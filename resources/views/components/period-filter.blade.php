@props(['filters', 'period', 'action' => null, 'ajax' => false, 'periodDescription' => 'All periods · Complete available data', 'hideAll' => false, 'hiddenParams' => []])

@php
    $typeLabels = ['daily' => 'Today', 'weekly' => 'Weekly', 'monthly' => 'Monthly', 'yearly' => 'Yearly'];
    $periodTypes = $filters['period_types'] ?? ['daily', 'weekly', 'monthly', 'yearly'];
@endphp
...
<div class="ppmu-period-filter ppmu-period-filter-ajax" id="kpiPeriodFilter" data-kpi-ajax-url="{{ $action }}">
    ...
    <div class="ppmu-period-filter-controls">
        <div class="ppmu-period-selects">
            <select data-filter="month" class="form-select form-select-sm ppmu-filter-month" data-period-control="weekly monthly all" @if(in_array($period['period_type'] ?? '', ['daily', 'yearly'])) hidden @endif>
                @foreach($filters['months'] as $value => $label)
                    <option value="{{ $value }}" @selected((string) ($period['month'] ?? now()->month) === (string) $value)>{{ $label }}</option>
                @endforeach
            </select>
            <select data-filter="year" class="form-select form-select-sm ppmu-filter-year" data-period-control="weekly monthly yearly all" @if(($period['period_type'] ?? '') === 'daily') hidden @endif>
