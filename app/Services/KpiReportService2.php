...
class KpiReportService
{
    ...
    private function applyInspectionFilters($query, array $filters)
    {
        return $query
            ...
            ->when(! empty($filters['year']), function ($q) use ($filters) {
                $q->whereYear('inspection_datetime', $filters['year']);
            })
            ->when(! empty($filters['period']) && empty($filters['date_from']) && empty($filters['date_to']), function ($q) use ($filters) {
                match ($filters['period']) {
                    'current_week' => $q->whereBetween('inspection_datetime', [now()->startOfWeek(), now()->endOfWeek()]),
                    'last_week' => $q->whereBetween('inspection_datetime', [now()->subWeek()->startOfWeek(), now()->subWeek()->endOfWeek()]),
                    'last_four_weeks' => $q->where('inspection_datetime', '>=', now()->subWeeks(4)->startOfDay()),
                    default => null,
                };
            })
    ...
    */
    ...
    public function getProvincialKpiSummary(array $filters): array
    {
        ...
        return [
            'total_records'      => $total,
            'districts_covered'  => (clone $query)->whereNotNull('district_id')->distinct('district_id')->count('district_id'),
            'categories_covered' => (clone $query)->whereNotNull('kpi_category_id')->distinct('kpi_category_id')->count('kpi_category_id'),
            'approved'           => (clone $query)->where('status', 'approved')->count(),
            'reviewed'           => (clone $query)->where('status', 'reviewed')->count(),
            'submitted'          => (clone $query)->where('status', 'submitted')->count(),
            'rejected'           => (clone $query)->where('status', 'rejected')->count(),
            'approval_rate'      => $total > 0 ? round(((clone $query)->where('status', 'approved')->count() / $total) * 100, 2) : 0,
        ];
    }
