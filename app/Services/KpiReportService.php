...
class KpiReportService
{
    ...
    */
    public function getGraphicalFilters(): array
    {
        ...
        return [
            ...
            // Keep data for backward compatibility, but UI should hide these.
    ...
    {
        $periodType = $this->normalizeGraphicalPeriodType($filters);

        // Weekly: resolve Thu->Wed PPMF week using week_no (YYYYWW).
        if ($periodType === 'weekly') {
            $weekNo = (string) ($filters['week_no'] ?? '');
            if ($weekNo !== '' && preg_match('/^\d{6}$/', $weekNo) === 1) {
                $range = app(ScorecardService::class)->getWeekDateRange($weekNo);
        ...
        // Monthly / yearly: date range derived from selected month/year.
        if ($periodType === 'monthly' && ! empty($filters['month']) && ! empty($filters['year'])) {
        ...
        if ($periodType === 'yearly' && ! empty($filters['year'])) {
            $year = (int) $filters['year'];
            if ($year >= 2000) {
        ...
        // Default: latest completed PPMF week from calendar.
        $default = app(ScorecardService::class)->getLatestCompletedPpmfWeekFilters();
        $weekNo = (string) ($default['week_no'] ?? '');
        $range = $weekNo !== '' ? app(ScorecardService::class)->getWeekDateRange($weekNo) : null;
        return [
            'period_type' => 'weekly',
            'date_from' => $range['start']?->toDateString(),
            'date_to' => $range['end']?->toDateString(),
            'label' => $range['label_with_year'] ?? 'Weekly',
        ];
    }

    private function normalizeGraphicalPeriodType(array $filters): string
    {
        $p = (string) ($filters['period_type'] ?? 'weekly');
        return in_array($p, ['weekly', 'monthly', 'yearly'], true) ? $p : 'weekly';
    }

    public function getGraphicalMetricCards(array $filters): array
    {
        $dateRange = $this->resolveGraphicalDateRange($filters);
        $periodType = (string) ($dateRange['period_type'] ?? $this->normalizeGraphicalPeriodType($filters));
        $dateFrom = $dateRange['date_from'] ?? null;
        $dateTo = $dateRange['date_to'] ?? null;
        $year = isset($filters['year']) ? (int) $filters['year'] : null;
        $weekNo = ! empty($filters['week_no']) ? (string) $filters['week_no'] : null;
        ...
        // Punjab / provincial scope: use kpi_metric_values (province)
        if (empty($filters['district_id'])) {
            $query = KpiMetricValue::query()
                ...
                ->when($periodType === 'monthly' && $year && $month, fn ($q) => $q->where('year', $year)->where('month', $month))
                ->when($periodType === 'yearly' && $year, fn ($q) => $q->where('year', $year))
                ->orderBy('sort_order')
                ->orderBy('id');

            if ($dateFrom && $dateTo) {
                $query->whereDate('date_from', $dateFrom)->whereDate('date_to', $dateTo);
            }
        ...
        // District scope: pull from kpi_metric_values (district)
        $districtId = $filters['district_id'];
        $rowsQuery = KpiMetricValue::query()
            ->where('is_active', true)
        ...
        if ($dateFrom && $dateTo) {
            $rowsQuery->whereDate('date_from', $dateFrom)->whereDate('date_to', $dateTo);
        }
        if ($periodType === 'weekly' && $weekNo) {
            $rowsQuery->where('week_no', $weekNo);
        }
        if ($periodType === 'monthly' && $year && $month) {
            $rowsQuery->where('year', $year)->where('month', $month);
        }
        if ($periodType === 'yearly' && $year) {
            $rowsQuery->where('year', $year);
        }
    ...
    public function buildGraphicalChartData(array $filters): array
    {
        ...
        // District-wise comparison charts use kpi_metric_values (district scope).
        $districtAggQuery = KpiMetricValue::query()
            ->select([
                'district_id',
        ...
        if ($periodType === 'weekly' && $weekNo) {
            $districtAggQuery->where('week_no', $weekNo);
        }
        if ($periodType === 'monthly' && $year && $month) {
            $districtAggQuery->where('year', $year)->where('month', $month);
        }
        if ($periodType === 'yearly' && $year) {
            $districtAggQuery->where('year', $year);
        }

        $districtAgg = $districtAggQuery->get();
        ...
        if ($isWater) {
            ...
            $fnAgg = KpiMetricValue::query()
                ->select([
                    'district_id',
                    DB::raw("SUM(CASE WHEN metric_key = 'functional' THEN metric_value ELSE 0 END) as functional"),
                    DB::raw("SUM(CASE WHEN metric_key = 'non_functional' THEN metric_value ELSE 0 END) as non_functional"),
                ])
                ->with('district:id,name')
                ->where('is_active', true)
                ->where('area_level', 'district')
                ...
                ->when($dateFrom && $dateTo, function ($q) use ($dateFrom, $dateTo) {
                    $q->whereDate('date_from', $dateFrom)->whereDate('date_to', $dateTo);
                })
                ->when($periodType === 'weekly' && $weekNo, fn ($q) => $q->where('week_no', $weekNo))
                ->groupBy('district_id')
        ...
        $nonFunctionalAgg = KpiMetricValue::query()
            ->select([
                'district_id',
                DB::raw("SUM(CASE WHEN metric_key = 'non_functional' THEN metric_value ELSE 0 END) as non_functional"),
            ])
        ...
        $charts['meta']['period_label'] = $dateRange['label'] ?? null;
        $charts['meta']['period_type'] = $periodType;
        $charts['meta']['date_from'] = $dateFrom;
        $charts['meta']['date_to'] = $dateTo;
    ...
    */
                ...
                if (! empty($filters['search'])) {
                    ...
                    $q->where(function ($qq) use ($search) {
                        $qq->where('metric_title', 'ILIKE', "%{$search}%")
                            ->orWhere('metric_key', 'ILIKE', "%{$search}%")
    ...
    public function getProvincialKpiMetricSummary(array $filters): array
    {
        ...
        $metricsQuery = KpiMetricValue::query()
            ->where('is_active', true)
            ->where('area_level', 'province')
            ->when(! empty($filters['kpi_category_id']), function ($q) use ($filters) {
                $q->where('kpi_category_id', $filters['kpi_category_id']);
    ...
    public function getDistrictWiseKpiScore(array $filters): array
    {
        ...
        $metricKeys = $metricCards->pluck('metric_key')->all();
        ...
        $valuesQuery = KpiMetricValue::query()
            ...
            ->when(! empty($metricKeys), function ($q) use ($metricKeys) {
                $q->whereIn('metric_key', $metricKeys);
            })
            ->when(! empty($filters['district_id']), function ($q) use ($filters) {
                $q->where('district_id', $filters['district_id']);
            })
        ...
        foreach ($grouped as $districtId => $items) {
            ...
            foreach ($items as $item) {
                $metrics[$item->metric_title] = [
                    ...
                    'badge_class' => $this->scoreToBadgeClass($item->metric_score !== null ? (float) $item->metric_score : null),
                    'evidence'    => null,
                ];
